...
import { X, Printer, Ban, RotateCcw, ShoppingBag, Laptop, UtensilsCrossed } from 'lucide-react'
...
interface Props { billId: string; onClose: () => void }

export function BillDetailModal({ billId, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center bg-black/50" onClick={onClose}>
      <div className="w-full md:w-[440px] bg-bg-surface rounded-t-2xl md:rounded-2xl flex flex-col max-h-[85vh] md:max-h-[80vh]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-sphotel-border shrink-0">
          <div className="flex items-center gap-2">
            ...
            {bill && <span className={`text-xs px-2 py-0.5 rounded-full ${bill.status === 'billed' ? 'bg-sphotel-accent-subtle text-sphotel-accent' : bill.status === 'cancelled' ? 'bg-text-muted/20 text-text-muted' : 'bg-status-error/10 text-status-error'}`}>{bill.status === 'billed' ? 'Settled' : bill.status === 'cancelled' ? 'Cancelled' : 'Void'}</span>}
          </div>
        ...
        {bill && <>
            ...
            {items.map((item) => (
              <div key={item.id} className="flex items-center justify-between py-2 border-b border-sphotel-border/40">
                ...
                <span className="text-sm font-medium text-text-primary shrink-0">
                  {fmt((item.override_price_paise ?? item.price_paise) * item.quantity)}
                </span>
              </div>
            ))}
          </div>
          <div className="px-4 py-3 border-t border-sphotel-border shrink-0 flex flex-col gap-1 text-sm">
            ...
            {bill.payment_method && <p className="text-xs text-text-muted mt-1 capitalize">Paid via {bill.payment_method}</p>}
          </div>
        </>}
      </div>
    </div>
