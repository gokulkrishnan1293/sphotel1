...
export function ActiveBillsPanel({ onSelect }: { onSelect?: () => void }) {
  ...
  return (
    <aside className="w-full md:w-56 shrink-0 bg-bg-surface border-r border-sphotel-border flex flex-col h-full">
      <div className="px-3 pt-4 pb-2 flex items-center justify-between shrink-0">
        <span className="text-xs font-medium text-text-muted uppercase tracking-wide">Open Bills</span>
        <button onClick={() => setShowNew(true)} className="w-6 h-6 flex items-center justify-center rounded-md bg-sphotel-accent text-sphotel-accent-fg hover:opacity-90" title="New Bill (N)"><Plus size={14} /></button>
      </div>
      ...
      <div className="shrink-0 border-t border-sphotel-border">
        <button onClick={() => setShowPast((v) => !v)} className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-text-muted hover:text-text-primary transition-colors">
