...
export function BillCanvas({ fontSizeIdx = 1, canOpenNewBill = true }: { fontSizeIdx?: number; canOpenNewBill?: boolean }) {
  ...
  const { activeBillId } = useBillingStore()
  ...
  useEffect(() => {
    ...
    const handle = (e: KeyboardEvent) => {
      ...
      if (matchKey(e, sc.close_bill)) { e.preventDefault(); if (bill && bill.status !== 'billed' && bill.status !== 'void' && bill.items.some((i) => i.status !== 'voided') && !closeBill.isPending) closeBill.mutate({ method: defaultMethod(bill.bill_type), discount: 0 }) }
    }
    window.addEventListener('keydown', handle)
    return () => window.removeEventListener('keydown', handle)
  }, [activeBillId, bill, closeBill.isPending])

  // No active bill — show inline bill creator
  if (!activeBillId) {
    return <InlineBillCreator canOpen={canOpenNewBill} />
  }
    ...
    <div className="flex-1 flex flex-col min-h-0">
      ...
      {!isClosed && (
        <InlineSearch
          ...
          onGenerateBill={() => { if (bill.status !== 'billed' && bill.status !== 'void') setSettleOpen(true) }}
        />
      )}
      <div className="flex-1 overflow-y-auto px-3 py-3 md:px-6 md:py-4 flex flex-col gap-0.5">
        ...
        {bill.items.length === 0 && <div className="flex flex-col items-center justify-center py-16"><p className="text-text-muted text-sm">Bill is empty — search above to add items</p></div>}
      </div>
      <BillFooter bill={bill} isBiller={isBiller} hasItems={hasItems}
        ...
        onAddItem={() => searchRef.current?.focus()} onFireKot={() => fireKot.mutate()} onSettle={() => setSettleOpen(true)}
