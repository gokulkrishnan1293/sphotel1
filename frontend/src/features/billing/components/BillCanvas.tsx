...
const fmt = (p: number) => `₹${(p / 100).toFixed(2)}`; const defaultMethod = (billType: string): PaymentMethod => billType === 'online' ? 'wallet' : 'cash'
...
export function BillCanvas() {
  ...
  const voidBill = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['bills', 'open'] }); useBillingStore.getState().setActiveBill(null) },
  })
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      {!isClosed && (
        <div className="px-6 py-4 border-t border-sphotel-border flex flex-col gap-3">
