...
import { X, Search, ArrowLeft, ShoppingBag, Laptop, UtensilsCrossed } from 'lucide-react'
import { billsApi } from '../api/bills'
import { BillDetailPanel } from './BillDetailModal'
...
export function PastBillsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50" onClick={onClose}>
      <div className="w-full md:w-[560px] bg-bg-surface rounded-t-2xl md:rounded-2xl flex flex-col max-h-[90vh] md:max-h-[85vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
        {/* Shared header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-sphotel-border shrink-0">
          {detailBillId
            ? <button onClick={() => setDetailBillId(null)} className="flex items-center gap-1.5 text-sm text-text-muted hover:text-text-primary transition-colors"><ArrowLeft size={15} /> Back</button>
        ...
        <div className="flex-1 relative overflow-hidden min-h-[420px]">
          {/* List page */}
          <div className={`absolute inset-0 flex flex-col transition-transform duration-300 ease-in-out ${detailBillId ? '-translate-x-full' : 'translate-x-0'}`}>
          ...
          {/* Detail page */}
          <div className={`absolute inset-0 flex flex-col transition-transform duration-300 ease-in-out ${detailBillId ? 'translate-x-0' : 'translate-x-full'}`}>
            {detailBillId && <BillDetailPanel billId={detailBillId} />}
          </div>
        </div>
      </div>
    </div>
